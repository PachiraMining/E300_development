...
#define PASSWORD_FILE "/var/www/html/lighttpd.user"
#define SESSION_FILE "/var/www/html/qcmap_session"
#define ATTEMPT_FILE "/var/www/html/login_attempt"
//Default and only user name for accessing web server
#define DEFAULT_USER_NAME "root"
#define DEFAULT_PASSWD "admin1234"
...
#define LOGIN_DISABLE_DURATION 180
#define CHECK_ATTEMPT 1
#define CLEAR_ATTEMPT 2
#define TOKEN_SIZE 16
#define SESSION_TOKEN_FILE "/var/www/html/session_token.txt"
#define TOKEN_GENERATE "hexdump -n16 /dev/urandom | tr -dc A-Z-a-z-0-9 | head -c16 \
                        > /var/www/html/session_token.txt"
#define SALT_GENERATE "hexdump -n16 /dev/urandom | tr -dc A-Z-a-z-0-9 | head -c16"
#define TOKEN_CLEAR "rm -rf /var/www/html/session_token.txt "
//changing permissions to restirct access to user and group only.
#define CHG_PERM_PASWD_FILE "chmod 770 /var/www/html/lighttpd.user"
//changing owner and group of file to root,as per security recommendations.
#define CHG_PERM_PASWD_FILE_OWNR "chown root:root /var/www/html/lighttpd.user"
#define CHG_PERM_ATTEMPT_FILE "chmod 770 /www/login_attempt"
#define CHG_PERM_ATTEMPT_FILE_OWNR "chown root:root /var/www/html/login_attempt"
#define CHG_PERM_SESSION_FILE "chmod 770 /var/www/html/qcmap_session"
#define CHG_PERM_SESSION_FILE_OWNR "chown root:root /var/www/html/qcmap_session"
#define CHG_SESSION_TOKEN_FILE "chmod 770 /var/www/html/session_token.txt"
#define CHG_SESSION_TOKEN_FILE_OWNR "chown root:root /var/www/html/session_token.txt"
#define MIN_PWD_LN 8
...
int unencode_string(char *src, int last);
...
//  int cgi_html_buff_len = 0;
  ...
  if (fp)
  {
    ...
    //change owner,group of file to root as per security recommendations.
  ...
  if (http_inc_traffic_len)
  {
    ...
    if(remote_addr)
    {
          ...
          strlen(http_element_val[0]))) == SUCCESS)
      {
        ...
        if (strncmp(sha512_code,pwd,MAX(strlen(sha512_code),strlen(pwd))) == SUCCESS)
        {
          ...
          if(fpe)
          {
            ...
            //change owner,group of file to root,
            //as per security recommendations.
        ...
        else
        {
          ...
          if (fp)
          {
            ...
            {
               // change Ip address to access devices
              fp = fopen(SESSION_FILE, "w+");
              if(fp)
              {
                //file access restrict to only owner,
      ...
      {
        //check username
        if ((strncmp(DEFAULT_USER_NAME,http_element_val[3], strlen(http_element_val[3])) != SUCCESS) || (strlen(DEFAULT_USER_NAME) != strlen(http_element_val[3])))
        {
          //Send failure HTTP responce
      ...
      //If request is to update password
      else if (strncmp("resetFactory",http_element_val[0],MAX(strlen("resetFactory"),
               strlen(http_element_val[0])))== SUCCESS)
      {
        //token validate starts
        fpe = fopen(SESSION_TOKEN_FILE, "r");
        ...
        if (fp)
        {
          ...
          else
          {
              ...
              if (fp)
              {
                ...
                //as per security recommendations.
                system(CHG_PERM_PASWD_FILE_OWNR);
                  //restore factory password in password file.
                  //calculate sha512 crypt of old password
                  if( ( sha512_code = sha512_crypt(http_element_val[1],sha512_salt_code))
                      == NULL )
                  {
                    ...
                    printf("%s",MD5_SUM_ERR);
                    fclose(fp);
//                    system("echo 8 > var/run/testFact");
                    printf("%s",FILE_OPEN_ERR);
                    return FAIL;
                  }
//                  system("echo 1 >> var/run/testFact");
                  fprintf(fp, "%s:%s",DEFAULT_USER_NAME,sha512_code);
                  fclose(fp);
//                  system("echo 9 > var/run/testFact");
//                  system("cp /data/factory_mobileap_cfg.xml /data/mobileap_cfg.xml");
//                  system("echo 2 >> var/run/testFact");
//                  system("rm -f /data/mobileap_firewall.xml");
//                  system("echo 3 >> var/run/testFact");
//                  system("cp /data/misc/wifi/factory_hostapd.conf /data/misc/wifi/hostapd.conf");
//                  system("echo 4 >> var/run/testFact");
//                  system("cp /data/misc/wifi/factory_hostapd-wlan1.conf /data/misc/wifi/hostapd-wlan1.conf");
//                  system("echo 5 >> var/run/testFact");
//                  system("reboot");
//                  system("echo 6 >> var/run/testFact");
//                  system("echo 1 > var/run/isResetFactory");
                  return SUCCESS;
              }
              else
              {
